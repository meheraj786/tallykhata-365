...
import { ArrowLeft, Phone, Trash2, Plus, Minus, Calendar, MoreVertical, Edit2, Clock } from "lucide-react";
...
export default function CustomerDetails() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "users", auth.currentUser.uid, "customers", id, "ledger"), orderBy("createdAt", "desc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setActiveLedger(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as any[]);
    });
    return () => unsubscribe();
  }, [id, setActiveLedger]);

  const handleEntry = (type: 'gave' | 'received') => {
    if (!id || !amount || !customer) return;
    addLedgerEntry(id, customer.name, {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pb-52">
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-6">
        <div className={`p-8 rounded-[2.5rem] text-center shadow-xl ${customer.totalDue >= 0 ? 'bg-emerald-600' : 'bg-rose-600'} text-white`}>
        ...
        <div className="space-y-3">
          {activeLedger.map((entry) => (
            <div key={entry.id} className="bg-white p-5 rounded-3xl flex items-center justify-between shadow-sm group transition-all">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-slate-400 uppercase flex items-center gap-1 mt-1"><Calendar size={10} /> {entry.date} <Clock size={10} className="ml-1" /> {entry.time}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <p className={`font-black text-lg ${entry.type === 'gave' ? 'text-rose-600' : 'text-emerald-600'}`}>৳{entry.amount.toLocaleString("bn-BD")}</p>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild><button className="p-2 text-slate-300 hover:text-slate-600"><MoreVertical size={20} /></button></DropdownMenuTrigger>
      ...
      <div className="fixed md:bottom-0 bottom-16 left-0 right-0 bg-white/95 backdrop-blur-xl border-t p-4 z-40 md:left-56">
        <div className="max-w-4xl mx-auto space-y-3">
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            ...
            <input type="date" value={entryDate} onChange={(e) => setEntryDate(e.target.value)} className="h-12 px-4 rounded-xl bg-slate-100 border-none font-bold outline-none" />
      ...
      <Dialog open={!!editingEntry} onOpenChange={() => setEditingEntry(null)}>
        <DialogContent className="rounded-[2rem] border-none p-8 bg-white max-w-[90vw] md:max-w-md shadow-2xl">
          <DialogHeader><DialogTitle className="text-xl font-black">লেনদেন সংশোধন</DialogTitle></DialogHeader>
          <div className="space-y-4 pt-4">
            ...
            <Button onClick={() => updateLedgerEntry(id!, editingEntry!.id, editingEntry!, editData).then(() => setEditingEntry(null))} className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-700 font-black mt-2">আপডেট করুন</Button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
